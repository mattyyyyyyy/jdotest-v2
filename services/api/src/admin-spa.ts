...
function api(m,u,b){var o={method:m,headers:{}};if(b){o.headers['Content-Type']='application/json';o.body=JSON.stringify(b);}return fetch(u,o).then(function(r){return r.json().catch(function(){return null;});});}
...
function renderShell(){
  ...
  s+="</nav><div class='jdo-sidebar-foot'><div class='jdo-avatar' style='width:30px;height:30px'>运</div><div style='line-height:1.3'><div style='font-size:13px;color:var(--txt-1);font-weight:600'>运营 · 李娜</div><div style='font-size:11px;color:var(--txt-3)'>yunying01@jdo</div></div></div>";
...
api('GET','/api/v1/admin/categories').then(function(cd){CATS=(cd&&cd.items)||[];CATS.forEach(function(c){CATMAP[c.id]=c.name;});return api('GET','/api/v1/admin/resources');}).then(function(d){RES=(d&&d.items)||[];renderShell();go('products');});
`;
