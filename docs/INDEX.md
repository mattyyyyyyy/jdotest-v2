...
|---|---|---|---|
| 2026-06-03 | claude-tdd-android | **Android TDD 牙齿：JaCoCo 门槛 + 仪器测试进 CI**：`app/build.gradle.kts` 加 `jacoco` 插件 + `enableUnitTestCoverage` + `jacocoUnitReport`/`jacocoUnitVerification`（**限定 data/Catalog 纯逻辑层**，避免 Compose UI 把比例拉到 0）；基线 data/Catalog 行覆盖 63.2%（60/95），门槛 line ≥ 0.60（可 `-PjacocoMin` 覆盖，实测 0.80→exit 1 咬人、0.60→exit 0）。CI：android-unit 改跑 `jacocoUnitVerification`；新增 **android-instrumented** job 用 `reactivecircus/android-emulator-runner`（api-34/x86_64/google_apis）起后端 + 跑 `connectedDebugAndroidTest`。本地实测：JVM 单测 + JaCoCo 绿、2 个仪器测试（BackendDataRender + PrimaryButton）在 emulator 通过。docs 同步：testing-strategy Android 行 ⬜→✅、缺口重排；ONBOARDING 划除两条缺口 | _pending_ |
| 2026-06-03 | claude-tdd-teeth | **admin（React）从 0→13 测试 + 覆盖率门槛**：装 Vitest + jsdom + Testing Library（@testing-library/react/jest-dom/user-event）；`apps/admin/vitest.config.ts`（jsdom + setup + thresholds）；`App.test.tsx`（fmt 纯函数 5 例 + 登录→控制台流程：成功进台显示角色/资源/数据行、失败显错不进台）+ `api.test.ts`（封装层：路径/鉴权头有无/错误 message 回退，mock 全局 fetch）；导出 `fmt` 以可测。**基线** Lines/Stmts 99.2%·Branch 87.8%·Funcs 78.6%（api.ts 100%），门槛 90/90/80/70。`test` 脚本带 `--coverage`，turbo `pnpm test` 5 任务全绿（admin 13 + api 106 + 状态机 20 = 139）。admin 不再是测试真空 | _pending_ |
| 2026-06-03 | claude-tdd-teeth | **给 TDD 装牙齿：覆盖率门槛进 CI（棘轮）**：装 `@vitest/coverage-v8@2.1.9`；`services/api` + `packages/order-state-machine` 各加 `vitest.config.ts`（provider v8 + include/exclude + thresholds）；`test` 脚本改 `vitest run --coverage`；`pnpm test`(CI 已在跑)即门槛 gate。**实测基线**：后端 Lines/Stmts 97.6%·Funcs 98.8%·Branch 80.4%，状态机 100%；门槛设基线略下（后端 95/95/95/78，状态机 90/90/90/85，只升不降）。**验证牙齿真咬**：抬高门槛 exit 1（CI 红）、正常 exit 0。`.gitignore` 忽略 `coverage/`；CI Test 步加注释；testing-strategy 标 ✅ 已落地。admin/Android 门槛仍待补 | _pending_ |
...
| 2026-06-03 | claude-finish-todos | **清空全部开放待办（Q2/Q15/Q14/Q11/Q13）**：①**Q2 持久化接 SQLite 实库**（better-sqlite3 同步，store_kv/store_meta 关系行，ADR-0014 升级，store 接口零改动）②**Q15 analytics 真实埋点**（`POST /events` + pv/uv(visitorId去重)/drivingSwitches 累加持久化，change archive）③**Q14 Android 测试**（JVM 单测 4 例 + Compose 仪器测试 1 例 emulator 通过 + 修 testInstrumentationRunner + CI android-unit job）④**Q11 _templates 重 vendoring**（按上游对齐 + 补 hooks/agents）⑤**Q13 apps/admin 独立工程**（Vite+React，登录+资源浏览，vite build 绿，进 turbo/CI）。**后端 98 测试 + order-machine 20 + admin/android 构建全绿，openspec 18/18**；后端切真 SQLite 运行（emulator 实测「立即购买」修复 + banner 自动左滑 3 卡） | `e17ec05` `c5f1a66` `7afe3fc` `6f769f8` `f7b81eb` |
| 2026-06-02 | claude-gap-fix | **项目欠缺系统性修复（硬伤+工程+口径+文档）**：①**三硬伤**——持久化(ADR-0014 JSON 快照,`STORE_PERSIST_PATH`,重启不丢)、库存校验(add-inventory-guard,加购/结算全有或全无+扣减)、消费端 mock 屏接 API(评价过滤hidden/收藏/可领券/售后,按userId隔离)；②**工程**——CI 四件套(`.github/workflows/ci.yml`)+`infra/docker-compose.yml`(PG/Redis)+`packages/api-contracts/openapi.yaml`+无依赖契约漂移守卫测试；③**口径统一安全子集**(ISO createdAt/updatedAt 自动盖 + data-dictionary 锁定 ID 前缀/枚举映射/sold 口径,不破坏前端)；④**文档对齐**——task-plan 严重漂移修正、open-questions 关 Q2/Q12 + 新增 Q13(apps/admin内嵌)/Q14(Android需SDK)/Q15(analytics埋点)、consistency-plan P0/P1/P2 全标注。**测试 56→95(+order-machine 20)全绿,18/18 specs,turbo 四件套绿** | `ad9811a` `17a769c` `1eb6105` `523740e` `14ac47c` +本次 |
...
| [ADR-0014](./decisions/ADR-0014-store-json-snapshot-persistence.md) | **store 持久化 = SQLite 实库**（better-sqlite3 同步驱动，`STORE_PERSIST_PATH` 指 `.db`，重启不丢；2026-06-03 由 JSON 快照升级；PG/ADR-0003 仍为生产目标；解 Q2）| Accepted | 2026-06-03 |
