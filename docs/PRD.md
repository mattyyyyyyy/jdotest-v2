...
我们做一个 **原生安卓车机电商平台（Kotlin + Jetpack Compose）**，以原生安卓应用形态运行在主流安卓车机/平板上，为车主、副驾、后排乘客提供"车上一键购"的完整购物体验。

关键解法：

- **原生安卓，车机优先**：Kotlin + Jetpack Compose 横屏车机为主、兼容平板/手机演示
...
车机原生安卓是新项目，无既有测试基础设施。**首个搭建测试脚手架的 PR 同时落地 OrderStateMachine 单测作为示范**，后续模块参照该 PR 风格。
