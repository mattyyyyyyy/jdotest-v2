...
# 编辑 openspec/** 后跑 openspec validate --all --strict；CLI 未安装则优雅跳过。
set -uo pipefail

# hook 在受限 PATH 下运行，主动把常见 node/openspec 位置补进来
export PATH="$HOME/.local/node20/bin:/opt/homebrew/bin:/usr/local/bin:$PATH"
...
if ! command -v openspec >/dev/null 2>&1; then
  echo "INFO(openspec): CLI 未安装，跳过校验。装法：npm i -g @fission-ai/openspec@latest（或本机 ~/.local/node20/bin）。" >&2
  exit 0
fi
