# admin-fulfillment Specification

## Purpose
后台履约管理：自提点 CRUD（含营业 open 启停）与物流轨迹录入，分角色权限点守卫（pickupPoints:write 运营 / shipping:write 客服）+ 审计。消费端履约读取接口待补（forward `add-fulfillment`）。
## Requirements
